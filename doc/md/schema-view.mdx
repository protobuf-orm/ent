...
func (CleanUser) Annotations() []schema.Annotation {
    return []schema.Annotation{
        ...
        entsql.View(`SELECT name, public_info FROM user`),
    }
}
...
func (CleanUser) Annotations() []schema.Annotation {
	return []schema.Annotation{
        entsql.View(`SELECT id, name, public_info FROM user`),
	}
}
...
```sql
-- Create "users" table
CREATE TABLE "user" ("id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY, "name" character varying NOT NULL, "public_info" character varying NOT NULL, "private_info" character varying NOT NULL, PRIMARY KEY ("id"));
-- Create "clean_users" view
CREATE VIEW "clean_users" ("id", "name", "public_info") AS SELECT id,
    name,
    public_info
