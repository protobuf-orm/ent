...
1\. Let's define a simple schema with two types (tables): `user` and `user_audit_log`:
...
Now, suppose we want to log every change to the `user` table and save it in the `user_audit_log` table.
To achieve this, we need to create a trigger function on `INSERT`, `UPDATE` and `DELETE` operations and attach it to
the `user` table.

2\. Next step, we define a trigger function ( `audit_users_changes`) and attach it to the `user` table using the `CREATE TRIGGER` commands:
...
BEGIN
    IF (TG_OP = 'INSERT') THEN
        INSERT INTO user_audit_log(operation_type, operation_time, new_value)
        VALUES (TG_OP, CURRENT_TIMESTAMP, row_to_json(NEW));
        RETURN NEW;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO user_audit_log(operation_type, operation_time, old_value, new_value)
        VALUES (TG_OP, CURRENT_TIMESTAMP, row_to_json(OLD), row_to_json(NEW));
        RETURN NEW;
    ELSIF (TG_OP = 'DELETE') THEN
        INSERT INTO user_audit_log(operation_type, operation_time, old_value)
...
the `user` and `user_audit_log` tables:

```sql
-- Create "user_audit_log" table
CREATE TABLE "user_audit_log" ("id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY, "operation_type" character varying NOT NULL, "operation_time" character varying NOT NULL, "old_value" character varying NULL, "new_value" character varying NULL, PRIMARY KEY ("id"));
-- Create "user" table
CREATE TABLE "user" ("id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY, "name" character varying NOT NULL, PRIMARY KEY ("id"));
...
CREATE TRIGGER "user_delete_audit" AFTER DELETE ON "user" FOR EACH ROW EXECUTE FUNCTION "audit_users_changes"();
-- Create trigger "user_insert_audit"
CREATE TRIGGER "user_insert_audit" AFTER INSERT ON "user" FOR EACH ROW EXECUTE FUNCTION "audit_users_changes"();
-- Create trigger "user_update_audit"
CREATE TRIGGER "user_update_audit" AFTER UPDATE ON "user" FOR EACH ROW EXECUTE FUNCTION "audit_users_changes"();
```
