...
UPDATE `user` SET `tags` = '["foo","bar"]' WHERE `tags` IS NULL OR JSON_CONTAINS(`tags`, 'null', '$');
```
...
-- Backfill all empty user names with default value 'unknown'.
UPDATE `user` SET `name` = 'Unknown' WHERE `user`.`name` = '';
```
...
UPDATE `user` SET `tags` = CASE WHEN (JSON_TYPE(JSON_EXTRACT(`tags`, '$')) IS NULL OR JSON_TYPE(JSON_EXTRACT(`tags`, '$')) = 'NULL') THEN JSON_ARRAY('org') ELSE JSON_ARRAY_APPEND(`tags`, '$', 'org') END WHERE (`user`.`name` LIKE 'org-%' OR `user`.`name` LIKE '%-org') AND (NOT (JSON_CONTAINS(`tags`, '"org"', '$') = 1));
```
...
-- Add the initial users to the database.
INSERT INTO `user` (`age`, `name`, `tags`) VALUES (1, 'a8m', '["foo"]'), (1, 'nati', '["bar"]');
```
