...
This is what makes the standard library UUID usable as a field type: it marshals to text, but implements neither
`driver.Valuer` nor `sql.Scanner`. Note that a generated package cannot mix it with another package named `uuid`,
such as `github.com/google/uuid`, because the two imports collide.
