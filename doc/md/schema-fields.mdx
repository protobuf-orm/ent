...
[ValueScanner](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field?tab=doc#ValueScanner) interface, or has an
...
:::note
At this stage, this option is available for text, numeric, UUID and JSON fields, but it will be extended to other
types in the future.
:::

A JSON column is written through `encoding/json`, so a `ValueScanner` on one has to hand back a value that is already
encoded. Returning a `json.RawMessage`, a `[]byte` or a `string` all work and are passed through as they are; anything
else is refused rather than encoded a second time. This is what lets a JSON column hold a type `encoding/json` cannot
read, such as a protobuf message generated with the opaque API, whose fields only `protojson` can see:

```go
field.JSON("profile", &Profile{}).
    ValueScanner(field.ValueScannerFunc[*Profile, *sql.NullString]{
        V: func(p *Profile) (driver.Value, error) { return protojson.Marshal(p) },
        S: func(ns *sql.NullString) (*Profile, error) {
            var p Profile
            return &p, protojson.Unmarshal([]byte(ns.String), &p)
        },
    })
```
...
`field.TextValueScanner` requires the type itself to implement both interfaces, which holds for pointer types like
`*big.Int`. Types that define `UnmarshalText` on the pointer receiver but are used by value, such as the `uuid.UUID`
of the standard library, use `field.TextValueScannerOf` instead. It takes the value type and works through its
pointer:
...
For a UUID field this is not needed at all. A `GoType` that marshals to and from text but implements neither
`driver.Valuer` nor `sql.Scanner` -- which is what the `uuid` package of the standard library is -- has its codec
written by codegen, so the field is only:
...
The type is `uuid.UUID` unless the schema names another one, which is what the second argument is for.

The codec cannot be installed by `field.Uuid` itself: the generated code asserts a `field.TypeValueScanner[T]`, and a
generic cannot be instantiated by reflection. Codegen knows the type statically and writes it there instead.

Note that a generated package cannot mix the standard library `uuid` with another package of the same name, such as
`github.com/google/uuid`, because the two imports collide.
...
[`entsql.Annotation`](https://pkg.go.dev/github.com/protobuf-orm/ent@master/dialect/entsql#Annotation):
...
It's mapped to a column name in SQL dialects.
...
When a custom [`GoType`](#go-type) is being used, it must be convertible to the basic `string` type or it needs to implement the [ValueScanner](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field#ValueScanner) interface.

The [EnumValues](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field#EnumValues) interface is also required by the custom Go type to tell Ent what are the permitted values of the enum. 
...
Implement the [EnumValues](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field#EnumValues) interface.
```go
package property
...
```
The following example shows how to define an `Enum` field with a custom Go type that is not convertible to `string`, but it implements the [ValueScanner](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field#ValueScanner) interface: 
...
```
Implement also the [ValueScanner](https://pkg.go.dev/github.com/protobuf-orm/ent/schema/field?tab=doc#ValueScanner) interface.
