...
If you manage schema changes with [Versioned Migrations](/docs/versioned-migrations), the migration directory Ent
writes can be linted before it reaches a database: replaying it on a throwaway database to catch invalid or
misordered statements, checking the [integrity file](/docs/versioned-migrations#atlas-migration-directory-integrity-file)
Ent maintains, and reporting destructive or data-dependent changes.

Linting is done by the Atlas CLI rather than by Ent. Its setup, including the GitHub action and the dev-database it
needs, is documented at [atlasgo.io/versioned/lint](https://atlasgo.io/versioned/lint).
